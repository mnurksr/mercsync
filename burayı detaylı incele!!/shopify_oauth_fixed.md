# SHOPIFY OAUTH WORKFLOW - CORRECTED VERSION
...
```json
{
  ...
  "connections": {
    ...
    "Generate Auth URL": {
      "main": [[{"node": "Redirect to Shopify", "type": "main", "index": 0}]]
    },
    "Callback Webhook": {
      "main": [[{"node": "Validate HMAC & Prepare", "type": "main", "index": 0}]]
    },
    ...
    "Shape for Database": {
      "main": [[{"node": "Upsert Shop (Postgres)", "type": "main", "index": 0}]]
    },
...
SHOPIFY_REDIRECT_URI=https://n8n.erenmuhammedortakvps.store/webhook/auth/shopify/callback
FRONTEND_URL=http://localhost:3000
```

## Database Migration (Run Before Testing)

```sql
-- Add last_token_refresh_at column if not exists
ALTER TABLE shops 
ADD COLUMN IF NOT EXISTS last_token_refresh_at timestamptz;
```

## TODO Before Production:
1. ✅ Implement Redis state storage for CSRF protection
2. ✅ Environment variables configured
3. ⚠️ Consider encrypting access_token in database (use Supabase Vault or pg_crypto)
