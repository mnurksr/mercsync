    ...
    try {
        ...
        for (const shop of shops) {
            ...
            try {
                ...
                const etsyData = await etsyApi.getListingsByState(shop.etsy_shop_id, 'active', shop.etsy_access_token, 100);
                
                if (!etsyData || !etsyData.results) continue;

                const recentListings = etsyData.results.filter((l: any) => l.last_modified_timestamp >= twoHoursAgo);
                console.log(`[Etsy Products Cron] Found ${recentListings.length} recently modified active listings for ${shop.shop_domain}`);
                ...
                for (const listing of recentListings) {
                    ...
                    if (matched && auto_update_products) {
                        try {
                            // Map product update via Shopify REST API
                            ...
                            // Minimal Update: Title & Description
                            ...
                            // Update price on the variant if needed
                            if (newPrice) {
                        ...
                        // Auto Create via cloneToShopify
                        ...
                        try {
                            ...
                            const cloneProduct = {
                                ...
                                variants: [{
                                    source_variant_id: listingId, // Default single variant for MVP
                                    title: 'Default Title',
                                    sku: listing.sku?.[0] || `ETSY-${listingId}`,
                                    price: cPrice,
                                    stock: listing.quantity || 1,
